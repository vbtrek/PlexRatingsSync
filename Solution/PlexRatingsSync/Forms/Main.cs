...
namespace DS.PlexRatingsSync
{
   public partial class Main : Form
   {
      ...
      private void cmdSync_Click(object sender, EventArgs e)
      {
         ...
         cmdSync.Text = "Cancel";
         chkSyncRatings.Enabled = false;
         chkSyncPlaylists.Enabled = false;
      ...
      private void bwProcess_DoWork(object sender, DoWorkEventArgs e)
      {
         ...
         if (!e.Cancel && m_PlexDb.IsDbConnected)
            Process();
      ...
      private void bwProcess_ProgressChanged(object sender, ProgressChangedEventArgs e)
      {
         ...
         {
            m_UpdateCount++;
            lblProgress.Text = $"Updated: {m_UpdateCount} | New: {m_AddCount}";
            lblProgress.Refresh();
            return;
         }
      ...
      {
         if (e.Cancelled)
            lblStatus.Text = "Sync was canceled";
         else
            lblStatus.Text = "Sync completed successfully";

         cmdSync.Text = "Start sync";
         chkSyncRatings.Enabled = true;
         chkSyncPlaylists.Enabled = true;
      }
...
WHERE id = {2}";
      ...
      private bool ValidateSettings()
      {
         ...
         {
            MessageBox.Show(this, "At least one sync option has to be selected", "Error", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            return false;
         }

         if (String.IsNullOrWhiteSpace(Settings.PlexDatabase))
         {
            MessageBox.Show(this, "Plex Database not configured", "Error", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            return false;
         }

         if (!File.Exists(Settings.PlexDatabase))
         {
            MessageBox.Show(this, $"Plex Database file not found :\n{Settings.PlexDatabase}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Stop);
            return false;
         }

         if (Settings.SyncPlaylists)
         {
            if (String.IsNullOrWhiteSpace(Settings.ItunesLibraryPath))
            {
               MessageBox.Show(this, "iTunes library path not configured", "Error", MessageBoxButtons.OK, MessageBoxIcon.Stop);
               return false;
            }

            if (!File.Exists(Settings.ItunesLibraryPath))
            {
               MessageBox.Show(this, $"iTunes library path not found :\n{Settings.ItunesLibraryPath}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Stop);
               return false;
            }
         }
